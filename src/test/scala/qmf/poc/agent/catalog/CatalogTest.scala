...
private val csn = "IBM-037"
private val cs = Charset.forName(csn)
class CatalogTest extends munit.FunSuite:
  test("get connection pool"):
    val ts = System.currentTimeMillis()
    ConnectionPool("jdbc:db2://qmfdb2.s4y.solutions:50000/sample", "db2inst1", "password", cs).test()
  ...
  test("get connection pool cache"):
    val ts = System.currentTimeMillis()
    ConnectionPool.memo("jdbc:db2://qmfdb2.s4y.solutions:50000/sample", "db2inst1", "password", csn).test()
    println(System.currentTimeMillis() - ts)
  ...
  test("json catalog object data"):
    val o = ObjectData("owner", "name", "type", 1, String(""))
    val ast = o.toJson
  ...
  test("get catalogs seq"):
    val o1 = ObjectData("owner", "name", "type", 1, String(""))
    val o2 = ObjectData("owner", "name", "type", 1, String(""))
    val ast = Seq(o1, o2).toJson
    val json = ast.compactPrint
    println(json)

  test("json catalogs"):
    val pool = ConnectionPool("jdbc:db2://qmfdb2.s4y.solutions:50000/sample", "db2inst1", "password", cs)
    val ts = System.currentTimeMillis()
    val catalog: Unit = CatalogProvider(pool).catalog match
      case Some(catalog) =>
