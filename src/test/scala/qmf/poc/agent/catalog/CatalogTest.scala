...
class CatalogTest extends munit.FunSuite:
  test("get connection pool"):
    ...
    ConnectionPool("jdbc:db2://qmfpoc.s4y.solutions:50000/sample", "db2inst1", "password").test()
  ...
  test("get connection pool cache"):
    val ts = System.currentTimeMillis()
    ConnectionPool.memo("jdbc:db2://qmfpoc.s4y.solutions:50000/sample", "db2inst1", "password").test()
    println(System.currentTimeMillis() - ts)
